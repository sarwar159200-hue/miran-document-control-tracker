...
import {readMsgFile,filterMsgFiles} from "../lib/msg-import";
...
export default function Page(){
 ...
 const [syncing,setSyncing]=useState(false),[syncMsg,setSyncMsg]=useState(""),[selectedPst,setSelectedPst]=useState(null),[readPct,setReadPct]=useState(0),[pstProgress,setPstProgress]=useState({parsed:0,processed:0,total:0,relevant:0,updates:0,needs:0});
 ...
 async function importPst(file=selectedPst){
   if(!file)return; setSyncing(true); setReadPct(0); setSyncMsg("Reading PST directly in your browser — the PST itself is never uploaded to Vercel or Supabase."); setPstProgress({parsed:0,processed:0,total:0,relevant:0,updates:0,needs:0});
   try{
     const messages=await readPst(file,p=>{
       if(p?.stage==='reading'){const pct=p.totalBytes?Math.min(100,Math.round((p.bytes/p.totalBytes)*100)):0;setReadPct(pct);setSyncMsg(`Reading ${file.name}… ${pct}%`);}
       if(p?.stage==='opening')setSyncMsg('PST file read successfully. Opening Outlook data…');
       if(p?.stage==='parsing'){setPstProgress(x=>({...x,parsed:p.messages||0}));setSyncMsg(`Scanning Outlook folders… ${p.messages||0} emails detected`);}
     });
     setReadPct(100); setPstProgress(x=>({...x,total:messages.length,parsed:messages.length}));
     ...
     for(let i=0;i<messages.length;i+=25){
       ...
       setPstProgress({parsed:messages.length,total:messages.length,processed,relevant,updates,needs}); setSyncMsg(`Importing extracted metadata… ${processed} / ${messages.length}`);
 ...
 async function importMsgFolder(files){
   const list=filterMsgFiles(files); if(!list.length){setSyncMsg('No Outlook .msg files were found in the selected folder.');return}
   setSyncing(true);setReadPct(0);setPstProgress({parsed:0,processed:0,total:list.length,relevant:0,updates:0,needs:0});setSyncMsg(`Large archive mode: ${list.length} Outlook messages selected. Processing one message at a time — files are not uploaded.`);
   let processed=0,relevant=0,updates=0,needs=0,failed=0;let batch=[];
   try{
     for(let i=0;i<list.length;i++){
       try{const m=await readMsgFile(list[i]);batch.push(m)}catch{failed++}
       setReadPct(Math.round(((i+1)/list.length)*100));setPstProgress(x=>({...x,parsed:i+1,total:list.length}));
       if(batch.length>=20||i===list.length-1){
         if(batch.length){const r=await fetch('/api/pst/import',{method:'POST',headers:{Authorization:`Bearer ${session.access_token}`,'content-type':'application/json'},body:JSON.stringify({messages:batch})});const j=await r.json();if(!r.ok)throw new Error(j.error||'Import failed');processed+=j.scanned||0;relevant+=j.relevant||0;updates+=j.register_updates||0;needs+=j.needs_review||0;batch=[];setPstProgress({parsed:i+1,total:list.length,processed,relevant,updates,needs});}
         setSyncMsg(`Large archive import: ${i+1}/${list.length} files read, ${processed} analyzed, ${relevant} relevant DCC, ${failed} unreadable.`);await new Promise(resolve=>setTimeout(resolve,0));
       }
     }
     setSyncMsg(`Historical import complete: ${processed} emails analyzed, ${relevant} relevant DCC emails, ${updates} register updates, ${needs} need review, ${failed} files could not be read.`);await loadData();
   }catch(e){setSyncMsg(`Import stopped: ${e.message}`)}finally{setSyncing(false)}
 }
 ...
 {view==="Historical Import"&&<div className="panel syncPanel"><div className="syncHead"><div><h3>Historical Outlook Import</h3><div className="small">No Microsoft Graph or Miran domain-admin approval required. MDCT stores only extracted metadata in Supabase.</div></div><span className="badge good">NO MICROSOFT ADMIN REQUIRED</span></div>
 <div className="panel inner"><h3>Option A — Standard PST</h3><div className="small">Use this only for PST files up to 750 MB. Large multi-GB PST files cannot be safely loaded into a normal browser memory space.</div><div className="uploadBox"><input type="file" accept=".pst,application/vnd.ms-outlook" disabled={syncing} onChange={e=>{const file=e.target.files?.[0]||null;setSelectedPst(file);setSyncMsg(file?`${file.name} selected (${(file.size/1024/1024).toFixed(1)} MB).`:'');setReadPct(0);setPstProgress({parsed:0,processed:0,total:0,relevant:0,updates:0,needs:0});}}/>{selectedPst&&<div className="fileReady"><b>{selectedPst.name}</b><span>{(selectedPst.size/1024/1024).toFixed(1)} MB</span>{selectedPst.size<=750*1024*1024?<button className="btn" disabled={syncing} onClick={()=>importPst(selectedPst)}>{syncing?'Processing…':'Start PST Analysis'}</button>:<span className="badge warn">USE LARGE ARCHIVE MODE BELOW</span>}</div>}</div></div>
 <div className="panel inner"><h3>Option B — Large Archive Mode (recommended for your 5.14 GB PST)</h3><p><b>Classic Outlook:</b> open your MDCT Historical folder → select all emails → drag them once into a normal Windows folder. Outlook creates individual <b>.msg</b> files. Then select that folder below. MDCT reads each message one-by-one in your browser, so a 5+ GB archive does not need to be loaded into memory.</p><div className="uploadBox"><input type="file" multiple webkitdirectory="" directory="" disabled={syncing} onChange={e=>{const files=filterMsgFiles(e.target.files);setMsgFiles(files);setSyncMsg(files.length?`${files.length} Outlook .msg files selected. Click Start Large Archive Analysis.`:'No .msg files found in that folder.');setReadPct(0);setPstProgress({parsed:0,processed:0,total:files.length,relevant:0,updates:0,needs:0});}}/>{msgFiles.length>0&&<div className="fileReady"><b>{msgFiles.length.toLocaleString()} Outlook messages</b><span>processed one at a time</span><button className="btn" disabled={syncing} onClick={()=>importMsgFolder(msgFiles)}>{syncing?'Processing…':'Start Large Archive Analysis'}</button></div>}</div></div>
 {syncing&&<div className="readProgress"><div style={{width:`${readPct}%`}}/><span>{readPct}%</span></div>}<div className="syncGrid"><div><div className="small">Emails Detected</div><b>{pstProgress.total||pstProgress.parsed}</b></div><div><div className="small">Processed</div><b>{pstProgress.processed}</b></div><div><div className="small">Relevant DCC</div><b>{pstProgress.relevant}</b></div><div><div className="small">Register Updates</div><b>{pstProgress.updates}</b></div><div><div className="small">Needs Review</div><b>{pstProgress.needs}</b></div></div>{syncMsg&&<div className="notice">{syncMsg}</div>}<div className="panel inner"><b>What MDCT analyzes automatically</b><p>Sender, subject, received date, forwarded email text, attachment filenames, Miran document numbers, transmittals, revisions, IFR / IFA / IFC / IFU / IFI / IFD / IFP, Code-1 to Code-4, contractor references and duplicate identifiers. Raw PST/MSG files, attachments and full email bodies are not stored in Supabase.</p></div></div>}
